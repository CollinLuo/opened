		...
		*/
        ...
        if ($M->where("`username`='" . $datas['username'] . "'")->count() >= 1) {
			...
			*/
			...
			//fwrites(APP_PATH . 'Admin/ajax.txt','-----数据库密码如下');
			//fwrites(APP_PATH . 'Admin/ajax.txt',$info['pwd']);
			//fwrites(APP_PATH . 'Admin/ajax.txt','-----post密码测试');
			//fwrites(APP_PATH . 'Admin/ajax.txt',$datas['pwd']);
			//fwrites(APP_PATH . 'Admin/ajax.txt','-----post密码加密测试');
			//fwrites(APP_PATH . 'Admin/ajax.txt',encrypt($datas['pwd']));
			//fwrites(APP_PATH . 'Admin/ajax.txt',"@param(auth)---info");
			//fwrites(APP_PATH . 'Admin/ajax.txt',$info);
            if ($info['pwd'] == encrypt($datas['pwd'])) {
				//fwrites(APP_PATH . 'Admin/ajax.txt','帐号密码正确！登录成功！');
                $loginMarked = C("TOKEN");
            ...
            return array('status' => 0, 'info' => "不存在帐号为：" . $datas["username"] . '的管理员账号！');
        }
    }
