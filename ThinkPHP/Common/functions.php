...
function p(){
	...
	echo '<div style="width:600px;text-align:left;margin-top:10px;margin-left:5px;margin-bottom:10px;"><pre>';
	//多个参数循环输出
	foreach($args as $arg){
		if(is_array($arg)){  
			print_r($arg);
 ...
 *  $ceshi = "需要写入的文件名";
 *  $strings = "需要写入的字符串内容";
 *  $times = "时间记录";  true 记录时间 false 不记录时间
 ***********************************************************/
 function fwrites($ceshi,$strings,$times=true){
	$files = fopen($ceshi,'a');
	if(!is_array($strings)){
		if($times = true){
			$times = "时间：" . date("Y-m-d H:i:s",time());
			fwrite($files,$times . "      " . $strings . "\n");
		}else{
			fwrite($files,$strings . "\n");
		}
	}else{
		$str = "\narray{\n";
		$nbsp = "        ";
		foreach($strings as $key => $value){
			if(is_array($value)){
				...
				foreach($value as $key1 => $value1){
					if(is_array($value1)){
						$str .= $nbsp .$nbsp .$key1 . " => " . $value1 . "\n";
						$str .= $nbsp .$nbsp ."{\n";
						foreach($value1 as $key2 => $value2){
							$str .= $nbsp .$nbsp .$nbsp .$key2 . " => " . $value2 . "\n";
						}
						$str .= $nbsp .$nbsp ."}\n";
					}else{
						$str .= $nbsp .$nbsp .$key1 . " => " . $value1 . "\n";
 ...
 */
function is_url($s){  
	$result = preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%
    =~_|]/i",$s);
	if($result){
		return true;
	}else{
